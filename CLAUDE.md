...
- **Non-trivial logic ships with a test.** If the ticket touches anything beyond config tweaks, doc edits, or one-line plumbing — algorithms, validators, parsers, persistence, auth, anything you'd second-guess in a code review — the PR must include a test (`backend/tests/test_*.py` or frontend equivalent) that exercises the AC. Manual `curl`/eyeball verification is not enough once the codebase has more than one moving part. The test stays in the suite; future PRs run it.
